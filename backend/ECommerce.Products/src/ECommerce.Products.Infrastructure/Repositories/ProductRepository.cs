...
public sealed class ProductRepository : IProductRepository
{
    ...
    {
        const string sql = "SELECT 1 FROM products WHERE code = @code LIMIT 1";
        using var conn = _factory.Create();
        var exists =
            await conn.ExecuteScalarAsync<int?>(new CommandDefinition(sql, new { code }, cancellationToken: ct));
    ...
    public async Task<Product?> GetByIdAsync(Guid id, CancellationToken ct)
    {
        ...
        using var conn = _factory.Create();
        return await conn.QueryFirstOrDefaultAsync<Product>(new CommandDefinition(sql, new { id },
    ...
    public async Task<Product?> GetByCodeAsync(string code, CancellationToken ct)
    {
        ...
        using var conn = _factory.Create();
        return await conn.QueryFirstOrDefaultAsync<Product>(new CommandDefinition(sql, new { code },
            cancellationToken: ct));
    }
    ...
    {
        const string sql =
            "UPDATE products SET deleted_at = NOW(), is_active = FALSE, updated_at = NOW() WHERE id = @id AND deleted_at IS NULL";
        using var conn = _factory.Create();
        await conn.ExecuteAsync(new CommandDefinition(sql, new { id }, cancellationToken: ct));
    }

    public async Task<(IReadOnlyList<Product>, int)> ListAsync(
        string? search, string? departmentCode, bool? isActive,
        int page, int pageSize, CancellationToken ct)
    {
        var filters = new List<string>();
        if (!string.IsNullOrWhiteSpace(search))
            filters.Add("(code ILIKE '%' || @search || '%' OR description ILIKE '%' || @search || '%')");
        if (!string.IsNullOrWhiteSpace(departmentCode))
            filters.Add("department_code = @departmentCode");
